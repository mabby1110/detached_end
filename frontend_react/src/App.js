...
function App() {
  ...
  const fetchUsersSOAP = async () => {
    try {
      ...
      const users = Array.from(xmlDoc.getElementsByTagName("user")).map(user => ({
  ...
  return (
    <div>
      ...
      <Container maxWidth="md" sx={{ mt: 4 }}>
        <Tabs value={tabValue} onChange={handleTabChange} aria-label="API tabs" centered>
          <Tab label="REST API" />
          <Tab label="SOAP API" />
        </Tabs>
        
        <Box sx={{ p: 3, border: 1, borderColor: 'divider', borderRadius: 1, mt: 2 }}>
          ...
          <List>
            {users.map(user => (
              <ListItem key={user.id} sx={{ borderBottom: 1, borderColor: 'grey.200' }}>
                <ListItemText 
                  primary={user.name} 
          ...
          <Box component="form" sx={{ mb: 3 }}>
            ...
            <Button 
              variant="contained" 
              onClick={tabValue === 0 ? createUser : createUserSOAP}
              sx={{ mt: 2 }}
            >
              Agregar Usuario
            </Button>
